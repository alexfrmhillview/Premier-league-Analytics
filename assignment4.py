...
def Predictor(df):
    #df = pd.read_csv("Football Players Stats (Premier League 2021-2022).csv", encoding='windows-1252')
    st.markdown("<h2 style='text-align: center; color: black;'> Goals Pedictor </h2>", unsafe_allow_html=True)
    st.markdown("""---""")
...
def line_graph(df):
    ...
    sns.lineplot(x="Age", y="Gls",data=df)
    st.pyplot(fig)
